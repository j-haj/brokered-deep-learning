...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Procedure{ProduceOffsprint}{$N_1, N_2$}
    \If{$|N_1| \neq |N_2|$}
  ...
  \EndIf
  \State \emph{// Possibly add connection}
  \If{$\texttt{sampleUniform}(0, 1) < \texttt{conn\_p}$}
  \State $n_{\text{conn}} \gets \texttt{UniformInt}(0, \texttt{self.n\_layers})$
  \For{$i = 0 \text{ to } n_{\text{conn}}$}
  \State Add random connection
  \EndFor
  \EndIf
  \EndProcedure
  \end{algorithmic}
